...
def train_model(
    ...
    weight_decays = [0.01, 0.005, 0.001, 0.0005, 0.0001]  # [0.01, 0.005, 0.001]
    ...
    for fold, (train_ids, val_ids) in tqdm(
        ...
        for p1, lr in enumerate(learning_rates):
            ...
            for p2, wd in enumerate(weight_decays):
                ...
                model = simple_NN(
                    num_features=n_features, num_output=n_classes, num_hidden=num_hidden
                )
                ...
                for epoch in range(num_epochs):
                    ...
                    valid_loss, valid_correct = valid_epoch_vae(
                        vae, model, device, val_dataloader
                    )
                    ...
                    if epoch + 1 % 10 == 0:
                        print(
                            "Train Loss: {tl}, Train Acc: {ta}, Val Loss: {vl}, Val Acc: {va}".format(
                                tl=train_loss, ta=train_acc, vl=valid_loss, va=valid_acc
                            )
                        )
                # Log matrix of hp:
    ...
    loss_df = pd.DataFrame(
        columns=["Epoch", "Train Loss", "Train Acc", "Test Loss", "Test Acc", "Fold"]
    )
    for fold in tqdm(range(n_folds), desc="Performing Cross-validation, outer"):
        ...
        optimizer = optim.Adam(
            model.parameters(),
            lr=learning_rates[lr_idx.item()],
            weight_decay=weight_decays[wd_idx.item()],
        )

        loss_df = pd.DataFrame(
            columns=["Train Loss", "Train Acc", "Test Loss", "Test Acc"],
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            test_loss, test_correct = valid_epoch_vae(
                vae, model, device, test_dataloader
            )
            ...
            if epoch + 1 % 10 == 0:
                print(
                    "Train Loss: {tl}, Train Acc: {ta}".format(
                        tl=train_loss, ta=train_acc
                    )
                )

        test_losses[fold], test_corrects[fold] = valid_epoch_vae(
            vae, model, device, test_dataloader
        )
    ...
    # Save loss dataframe:
    loss_df.to_pickle(
        savefolder + "/" + "loss_df_{num_hidden}.pkl".format(num_hidden=num_hidden[0])
    )

    print(
        "Fold number {n}, Test Loss Mean, Std: {telm}, {tels}, Test Acc Mean, Std: {team}, {teas}".format(
            n=fold,
            telm=np.mean(test_loss),
            tels=np.std(test_loss),
            team=np.mean(test_acc_finals),
            teas=np.std(test_acc_finals),
        )
    )

    return (
        np.mean(test_acc_finals),
        np.std(test_acc_finals),
        best_acc,
        learning_rates[lr_idx.item()],
        weight_decays[wd_idx.item()],
    )


def main():
    # Implemented with better VAAL features
    dataset_size = 1600
    train_epochs = 300  # Change
    train_epochs_vae = 150  # Change
    lr_vae = 5e-04
    beta = 1
    batch_size = 32
    batch_size_vae = 32
    ...
    train_set = ESC50(
        ...
        audio_dir=to_absolute_path(
            "data/processed/ESC50/train_{n}x{n}".format(n=dataset_dimension)
        ),
        DR=False,
    )
    ...
    test_set = ESC50(
        ...
        audio_dir=to_absolute_path(
            "data/processed/ESC50/test_{n}x{n}".format(n=dataset_dimension)
        ),
        DR=False,
    )
    ...
    test_dataloader = DataLoader(test_set, batch_size=400, drop_last=False,)

    # Make sure we can save our results from training the final models:

    savefolder = "outputs/test_params_simple_vae"
    ...
    hidden_combinations = [[64, 32]]
    fold_results = pd.DataFrame(
        columns=["LR", "WD", "Mean Acc over folds", "Mean Final Acc", "Std Final Acc"],
        index=[str(config) for config in hidden_combinations],
    )
    for num_hidden in hidden_combinations:
        print(
            "Using {first} neurons in first layer and {second} neurons in second layer".format(
                first=num_hidden[0], second=num_hidden[1]
            )
        )
        ...
        fold_results.loc[str(num_hidden)] = [
            lr,
            wd,
            best_acc,
            test_acc_mean,
        ...
        print(
            "Test Accuracy for {config}: {acc}".format(
                config=str(num_hidden), acc=test_acc_mean
            )
        )
    # Save this dataframe:
    fold_results.to_pickle(
        savefolder
        + "/"
        + "fold_results_simple_vae_{model}.pkl".format(model=hidden_combinations[0][0])
    )
